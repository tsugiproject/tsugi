...
class TsugiCourses extends LitElement {
    ...
    async loadData() {
        ...
        try {
            ...
            if (!response.ok) {
                this.error = response.status === 403 ? 'Not available for this session.' : 'Could not load courses.';
                this.courses = [];
                return;
            }
            ...
            if (Array.isArray(data)) {
                ...
                if (data.show_catalog && data.catalog_url) {
                    this.catalogUrl = String(data.catalog_url);
                }
            ...
            this.courses = rows.map((row) => ({
                ...
                title: row.title || ('Course ' + (row.context_id || '')),
        ...
        return html`
            ...
            ${this.popupOpen ? html`
                <div class="popup ${this.popupAlign === 'left' ? 'align-left' : 'align-right'}">
                    ...
                    ${!this.loading && this.courses && this.courses.length > 0 ? html`
                        <ul class="course-list">
                            ${this.courses.map((c) => html`
                                <li>
                                    <a href=${c.href || this.courseHref(c.id)} class=${c.id === this.currentContextId ? 'current' : ''}>
                    ...
                    ` : ''}
                    ${(!this.loading && this.canCreate) || this.allUrl || this.catalogUrl ? html`
                        <div class="popup-footer">
                            ...
                            ${this.catalogUrl ? html`<a href=${this.catalogUrl}>Course catalog</a>` : ''}
                        </div>
                    ` : ''}
                </div>
            ` : ''}
