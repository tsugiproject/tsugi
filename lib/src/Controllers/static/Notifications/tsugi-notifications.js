...
class TsugiNotifications extends LitElement {
    ...
    updated(changed) {
        if (this.popupOpen && (changed.has('popupOpen') || changed.has('notifications') || changed.has('announcements'))) {
            this.positionPopup();
        }
    }
    ...
    positionPopup() {
        ...
        const popupWidth = Math.max(popup.offsetWidth || 0, 300);
        const margin = 8;
        
        popup.classList.remove('align-right', 'align-left');
        
        if (host.left + popupWidth + margin > window.innerWidth) {
