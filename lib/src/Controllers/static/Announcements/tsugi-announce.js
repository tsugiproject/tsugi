...
class TsugiAnnounce extends LitElement {
    ...
    notifyNotificationsComponent() {
        // Dispatch custom event to notify notifications component
        const event = new CustomEvent('announcement-dismissed', {
            detail: {
                dismissedCount: this.undismissedCount
            },
            bubbles: true,
            cancelable: true
        });
        ...
        // Also use BroadcastChannel if available for cross-component communication
        if ('BroadcastChannel' in window) {
            if (!this.broadcastChannel) {
                this.broadcastChannel = new BroadcastChannel('announcements-updates');
            }
            this.broadcastChannel.postMessage({
                type: 'announcement-dismissed',
                dismissedCount: this.undismissedCount
            });
        }
    }
    ...
    async loadNotifications() {
        if (!this.notificationsUrl) return;
        
        try {
            const jsonUrl = this.notificationsUrl + '/json';
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                if (announcement) {
                    ...
                    // Notify notifications component to refresh its badge count
                    this.notifyNotificationsComponent();
                }
            }
        } catch (error) {
    ...
    render() {
        ...
        const displayNotifications = [...unreadNotifications, ...readNotifications].slice(0, 10); // Show up to 10 most recent
        
        // Total badge count includes announcements, notifications, and push notifications
        const totalBadgeCount = this.undismissedCount + this.unreadNotificationCount + (this.pushNotificationCount || 0);
        
        const hasAnyContent = finalAnnouncements.length > 0 || displayNotifications.length > 0 || (this.pushNotificationCount > 0);
            ...
            ${this.popupOpen ? html`
                <div class="popup">
                    ...
                    <div class="popup-content">
                        ${!hasAnyContent ? html`
                            <div class="empty-message">No notifications or announcements</div>
                        ` : html`
                            <div class="announcement-list">
                                ...
                                ` : ''}
                                ${displayNotifications.map(notification => html`
                                    <div class="announcement-item ${notification.is_read ? 'dismissed' : ''}">
                                        <a href="${notification.url || this.notificationsUrl}" class="announcement-title">
                                            ${notification.is_read ? '' : html`<strong>`}${notification.title}${notification.is_read ? '' : html`</strong>`}
                                        </a>
                                    </div>
                                `)}
