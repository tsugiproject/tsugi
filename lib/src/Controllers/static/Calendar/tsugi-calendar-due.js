 ...
 *       lessons-url="/app/lessons">
 *   </tsugi-calendar-due>
 *
 * When due_soon + late > 0: shows a calendar icon with a badge (sum) and a popup
 * with counts plus a link to lessons. When both are zero: calendar icon is a plain
 * link to lessons (no badge, no popup).
 *
...
class TsugiCalendarDue extends LitElement {
    ...
    render() {
        const lessons = this.lessonsUrl || '/lessons';
        const icon = html`
            <svg
                class="calendar-icon"
        ...
        if (this.totalAlert < 1) {
            return html`
                <a class="calendar-link calendar-icon-container" href="${lessons}" title="Lessons" aria-label="Lessons">
                    ${icon}
                </a>
            `;
        }
        ...
        return html`
            <span
                ...
                role="button"
                tabindex="0"
                title="Assignment due summary"
                aria-label="Assignment due summary, ${total} total"
            ...
            ${this.popupOpen
                ? html`
                      <div class="popup" role="dialog" aria-label="Due assignments summary">
                          ...
                          <div class="popup-body">
                              ...
                              <a class="popup-lessons" href="${lessons}">Lessons</a>
                          </div>
                      </div>
                  `
                : ''}
