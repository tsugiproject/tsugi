...
class Mersenne_TwisterTest extends \PHPUnit\Framework\TestCase
{
    public function testGeneral() {
        ...
        for($i=0; $i < 30; $i++ ) {
            $ran = $MT->getNext(0,6);
	    $this->assertEquals($ran, $should_return[$i]);
        ...
        // for($i=0;$i<100;$i++) echo($hist[$i].' '); echo("\n");
        $ave = $tot / $cnt;
	...
	$this->assertNotEquals(PHP_INT_MAX, 2147483647);
