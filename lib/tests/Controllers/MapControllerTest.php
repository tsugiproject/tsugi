...
class MapControllerTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Test that Map::routes() registers routes correctly
     */
    public function testRoutesRegistersCorrectRoutes()
    {
        // Register routes
        Map::routes($this->mockApp);
        ...
        $this->assertTrue($hasMapRoute, 'Should register /map route');
        
        // Should have /map/json route
        $hasMapJsonRoute = false;
        foreach ($uris as $uri) {
            if (strpos($uri, '/map/json') === 0) {
                $hasMapJsonRoute = true;
                break;
            }
        }
        $this->assertTrue($hasMapJsonRoute, 'Should register /map/json route');
    }
    ...
    {
        $this->assertEquals('/map', Map::ROUTE, 'ROUTE constant should be /map');
    }
}
