...
class Context extends Entity {
     ...
     *
     */
    private function loadLTI13Data(&$lti13_token_url, &$lti13_privkey, &$issuer_client_id)
    {
        ...
        $issuer_client_id = $this->launch->ltiParameter('issuer_client_id');

        $missing = '';
        if ( strlen($issuer_client_id) < 1 ) $missing .= ' ' . 'issuer_client_id';
    ...
    public function loadNamesAndRoles($with_sourcedids=false, &$debug_log=false) {
        ...
        $missing = $this->loadLTI13Data($lti13_token_url, $lti13_privkey, $issuer_client_id);
        ...
        if ( $with_sourcedids ) {
            $nrps_access_token = LTI13::getNRPSWithSourceDidsToken($CFG->wwwroot, $issuer_client_id, $lti13_token_url, $lti13_privkey, $debug_log);
        } else {
            $nrps_access_token = LTI13::getNRPSToken($CFG->wwwroot, $issuer_client_id, $lti13_token_url, $lti13_privkey, $debug_log);
        }
    ...
    public function getLineItemsToken(&$missing, &$lti13_lineitems, &$debug_log=false)
    {
        ...
        // TODO: In the future we might cache this access token perhaps in session for a while
        $lineitems_access_token = LTI13::getLineItemsToken($CFG->wwwroot, $issuer_client_id, $lti13_token_url, $lti13_privkey, $debug_log);
    ...
    public function getGradeToken(&$missing, &$subject, &$debug_log=false)
    {
        ...
        // TODO: This used to be key_key and now we changed it to $issuer_client_id
        ...
        // TODO: Make sure issuer_client_id works
        // $grade_token = LTI13::getGradeToken($CFG->wwwroot, $subject, $lti13_token_url, $lti13_privkey, $debug_log);

        // TODO: In the future we might cache this access token perhaps in session for a while
        $grade_token = LTI13::getGradeToken($CFG->wwwroot, $issuer_client_id, $lti13_token_url, $lti13_privkey, $debug_log);
        return $grade_token;
    }
