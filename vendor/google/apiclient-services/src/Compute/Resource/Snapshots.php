...
use Google\Service\Compute\SnapshotUpdateKmsKeyRequest;
...
class Snapshots extends \Google\Service\Resource
{
   ...
   * specified snapshot. (snapshots.updateKmsKey)
   *
   * @param string $project Project ID for this request.
   * @param string $snapshot Name of the snapshot resource to update. Should
   * conform to RFC1035.
   * @param SnapshotUpdateKmsKeyRequest $postBody
   * @param array $optParams Optional parameters.
   *
   * @opt_param string requestId An optional request ID to identify requests.
   * Specify a unique request ID so that if you must retry your request, the
   ...
   * @throws \Google\Service\Exception
   */
  public function updateKmsKey($project, $snapshot, SnapshotUpdateKmsKeyRequest $postBody, $optParams = [])
  {
    $params = ['project' => $project, 'snapshot' => $snapshot, 'postBody' => $postBody];
    $params = array_merge($params, $optParams);
