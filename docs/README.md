# Tsugi documentation

Cross-cutting setup and operations guides. Feature-specific READMEs remain next to
their code (for example `tool/`, `admin/`).

| Guide | Topic |
|-------|--------|
| [composer.md](composer.md) | Composer updates, committed `vendor/`, pre-commit checks |
| [vapid.md](vapid.md) | Web push notification VAPID keys |
| [stripe.md](stripe.md) | Stripe Checkout supporter payments |
| [badges-assertions.md](badges-assertions.md) | Open Badges 2.0 / 3.0 assertion endpoints and config |
| [badges-linkedin.md](badges-linkedin.md) | LinkedIn credential sharing (PY4E example) |

Related docs elsewhere in the tree:

- [AGENTS.md](../AGENTS.md) — repository guidelines for contributors and agents
- [CONTRIBUTING.md](../CONTRIBUTING.md) — contribution workflow
